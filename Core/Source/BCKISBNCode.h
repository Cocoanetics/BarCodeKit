...
/**
 ...
 @param content The ISBN10/13 string including hyphens separating each ISBN element, must include the ISBN check digit, but WITHOUT the "ISBN" prefix. For example `978-3-16-148410-0` or `3-16-148410-X`.
 @param error Optional output parameter to take an `NSError` in case the content cannot be encoded by this barcode class. Pass `nil` if not error information is not required.
