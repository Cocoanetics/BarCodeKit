...
/**
 ...
 - one of two initialisers accepting the ISBN code's elements, one with and one without the ISBN check digit.
 
 The ISBN10 check digit is **not** always identical to the EAN13 check digit. This is because the ISBN10 check digit (if provided to the initialiser) is ignored during the conversion to ISBN13 and EAN13 encoding. However, the ISBN title text shown above the barcode image shows the ISBN10 character string with the original ISBN10 check digit. BCKISBNCode will validate any check digits provided and reject the character string if the check digit is invalid.
 ...
 BCKISBNCode validates ISBN strings by validating them against the International ISBN Agency ranges but not any of the country and language specific ranges.
